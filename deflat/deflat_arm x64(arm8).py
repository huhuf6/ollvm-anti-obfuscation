    ...
    for relevant in relevants_without_retn:
        ...
        for ins in block.capstone.insns:
            if project.arch.name in ARCH_X86:
                ...
                if ins.insn.mnemonic != 'movw' and ins.insn.mnemonic != 'mov' and ins.insn.mnemonic.startswith('mov'):
                    if relevant not in patch_instrs:
                        patch_instrs[relevant] = ins
                        has_branches = True
        ...
        if has_branches:
            flow[relevant].append(symbolic_execution(project, relevant_block_addrs,relevant.addr, hook_addrs, claripy.BVV(1, 1), True))
            flow[relevant].append(symbolic_execution(project, relevant_block_addrs,relevant.addr, hook_addrs, claripy.BVV(0, 1), True))
        else:
            flow[relevant].append(symbolic_execution(project, relevant_block_addrs,relevant.addr, hook_addrs))
